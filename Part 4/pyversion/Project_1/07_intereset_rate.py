...
class Account:
    ...
    # We haven't studied how to write class properties (we'll see how when we look at descriptors),
    # so for now we could just use a bare attribute - not ideal, but let's try that first.
    # interest_rate = 0.5 # percentage

    # If we don't feel comfortable with the bare attribute and want to validate changing the interest rate,
    # then we could use a more Java-like approach with getter and setter class methods.
    # We'll start by changing the class variable name to indicate it is a "private" attribute,
    # then add a getter and a setter class method:
